...
- [x] OG image: `public/og.png` (1200×630) is generated by `node scripts/build-og.mjs`
      and is already the default in `Base.astro`. Re-run that script after changing the
      headline or the cert line; `public/og.svg` is its source, not the shipped card.
- [x] Resume: `public/blake-grosskopf-resume.pdf` is live. **This hosted copy is a
      phone-number-stripped variant** for privacy — the full resume with the phone number
      lives at `personal/Blake Grosskopf Resume 2026.pdf` (for direct job-portal
      applications), NOT on the site. ⚠️ When updating the resume, re-export the personal
      docx, then make the web copy by removing the `  |  (817) …` phone segment from the
      contact line before exporting to `public/`. Do not copy the personal PDF over the
      hosted one — that re-leaks the phone. `public/_headers` also sets `X-Robots-Tag:
      noindex` on the PDF so it stays out of search results (Cloudflare Pages honors it).
