...
takeaway: "I can carry a cloud attack from anonymous public exposure to subscription-level control end to end, reason about why each step works at the platform level, improvise an equivalent path when the intended one is blocked, and translate the whole chain into concrete defenses."
...
killchain:
  ...
  - { title: "SSRF → file://", mitre: "T1190", live: true, detail: "IMDS was empty (no VM metadata on Functions), so I pivoted to file:///proc/self/environ to dump the environment." }
  ...
  - { title: "RG Contributor", mitre: "Impact", detail: "Replayed the token against ARM for Contributor over the resource group." }
findings:
...
and then attacked purely from the outside: no portal, no credentials, and no insider
...
don't break." None of those are exotic bugs. They are the ordinary shortcuts real cloud
...
naming convention for the storage account. One container was **Blob-level public**, so I
...
protected route: no registration, no password, no MFA anywhere in it.
...
`169.254.169.254`, returned nothing, because **Azure Functions has no VM metadata
service** and this function had no managed identity at all. That negative result is the
lesson: *the metadata endpoint you target has to match the compute type.* Instead I
pointed the SSRF at **`file:///proc/self/environ`** and dumped the function's entire
environment, confirming the live secrets and leaking a read/write storage SAS as a bonus.

The stolen storage-account key then unlocked the **whole** account, including containers
that Blob-level public access had kept hidden. One of them held **SSH private keys** for the
...
- **Treat every user-supplied URL as hostile.** Allow-list outbound destinations and
  strip `file://` and `gopher://` schemes, and SSRF stops being a credential-exfil primitive.
...
Azure Resource Manager REST API directly with `az rest` to route around CLI wrapper crashes,
...
exercise in reliable cloud tooling under constraint, which is the kind of friction that
shows up in every genuine engagement.
